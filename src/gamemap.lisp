...
(defclass gamemap ()
  ((gamemap-size :initarg :gamemap-size :initform (vec2 30 18) :accessor gamemap-size)
   ...
   (gamemap-glyph-array :initform (make-array 20 :adjustable t :fill-pointer t))
   (gamemap-mapcell :initform nil :accessor gamemap-mapcell)))

(defmethod initialize-instance :after ((gamemap gamemap))
  (with-slots (gamemap-gamecell gamemap-size) gamemap
    (setf gamemap-gamecell (make-hash-table))))
...
(defmethod destroy-gamecell-at-yx ((gamemap gamemap) (game-position vec2))
  ())
...
(defmacro with-mapcell-at-yx-ensure (gamemap game-position mapcell &body body)
  `(progn
     (init-gamecell-at-yx ,gamemap ,game-position)
     (with-slots (gamemap-mapcell) ,gamemap
       (let ((,mapcell (gethash (list (vy ,game-position) (vx ,game-position)))))
         ,@body))))
...
(defmethod add-glyph-to-mapcell ((gamemap gamemap) (glyph glyph))
  (let ((game-position glyph-game-position glyph))
