...
;; 注意这里是 vec2
(defmethod camera-viewport-render-char ((camera camera) (char character) (position vec2))
  "渲染单个char"
...
;; 注意这里是 vec3
(defmethod camera-render-char ((camera camera) (char character) (position vec3))
  "渲染单个char"
  (let* ((shadowcasting-position (camera-compute-shadowcasting-position camera position))
    ...
    (camera-viewport-render-char camera char render-position)))

(defmethod camera-render-target-axis ((camera camera))
  "渲染camera的准星"
  (camera-render-char camera #\+ (camera-target-position camera)))
...
(defmethod camera-render-glyph ((camera camera) (glyph glyph))
  ...
  (loop :for index :from 0
        ...
        :do (let ((glyph-char-position (v+ (dungeon/glyph::glyph-position glyph) (v* index (vec3 0 0 *glyph-size*)))))
              (camera-render-char camera c glyph-char-position))))
